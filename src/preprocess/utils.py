...
def insert_overwrite_partition_by_date_model(spark, df, path, col_partition_date='date', schema=None):
    jvm = spark._jvm
    jsc = spark._jsc
    fs = jvm.org.apache.hadoop.fs.FileSystem.get(jsc.hadoopConfiguration())
    cols = [c for c in df.columns if c != col_partition_date]
    partition_list = df[col_partition_date].unique()

    for p in partition_list:
        df_filter = df[df[col_partition_date] == p]

        for m in df_filter['model'].unique():
            df_filter_2 = df_filter[df_filter['model'] == m]
            df_spark = spark.createDataFrame(df_filter_2[cols], schema=schema)
            df_spark.write.mode('overwrite').format('avro').save(path + '/' + col_partition_date + '=' + str(p))